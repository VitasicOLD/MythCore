...
uint32 Unit::DealDamage(Unit* pVictim, uint32 damage, CleanDamage const* pCleanDMG, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* pSpellEntry, bool durabilityLoss)
{
    if(!pVictim)
        return 0;
    ...
    if(damagetype != NODAMAGE)
    {
        ...
        if(pSpellEntry)
        {
            if(!(pSpellEntry->AttributesEx4 & SPELL_ATTR4_DAMAGE_DOESNT_BREAK_AURAS))
                pVictim->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_TAKE_DAMAGE, pSpellEntry->Id);
    ...
    {
        pPlayer = pVictim->GetCharmer()->ToPlayer();
    ...
    if(health <= damage)
    {
        if(pVictim->GetTypeId() == TYPEID_PLAYER && pVictim != this)
        {
            ...
            if(pPlayer = GetCharmerOrOwnerPlayerOrPlayerItself())
        ...
        if(damagetype == DIRECT_DAMAGE || damagetype == SPELL_DIRECT_DAMAGE)
            pVictim->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_DIRECT_DAMAGE, pSpellEntry ? pSpellEntry->Id : 0);

        if(pVictim->GetTypeId() != TYPEID_PLAYER)
            pVictim->AddThreat(this, (float)damage, damageSchoolMask, pSpellEntry);
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    // battleground things (do this at the end, so the death state flag will be properly set to handle in the pBG->handlekill)
    if(player && player->InBattleground())
    {
        if(Battleground* pBG = player->GetBattleground())
        {
            if(victim->GetTypeId() == TYPEID_PLAYER)
                pBG->HandleKillPlayer((Player*)victim, player);
            else
                pBG->HandleKillUnit(victim->ToCreature(), player);
        }
    }
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
    ...
    if(Player* plr = ToPlayer())
    {
        ...
        if(Battleground* pBG = plr->GetBattleground())
            pBG->EventPlayerDroppedFlag(plr);
