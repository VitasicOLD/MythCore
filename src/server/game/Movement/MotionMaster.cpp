...
void MotionMaster::MoveTargetedHome()
{
    ...
    if(i_owner->GetTypeId() == TYPEID_UNIT)
    {
        ...
        else
        {
            ...
            if(pTarget)
            {
                sLog->outStaticDebug("Following %s (GUID: %u)", pTarget->GetTypeId() == TYPEID_PLAYER ? "player" : "creature", pTarget->GetTypeId() == TYPEID_PLAYER ? pTarget->GetGUIDLow() : ((Creature*)pTarget)->GetDBTableGUIDLow());
