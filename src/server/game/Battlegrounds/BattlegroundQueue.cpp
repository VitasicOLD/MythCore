...
void BattlegroundQueue::RemovePlayer(const uint64& guid, bool decreaseInvitedCount)
{
    ...
    {
        Battleground* pBG = sBattlegroundMgr->GetBattleground(group->IsInvitedToBGInstanceGUID, group->BgTypeId == BATTLEGROUND_AA ? BATTLEGROUND_TYPE_NONE : group->BgTypeId);
        if(pBG)
    ...
    {
        ArenaTeam* pAT = sArenaTeamMgr->GetArenaTeamById(group->ArenaTeamId);
        if(pAT)
        {
