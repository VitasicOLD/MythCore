...
bool Map::Add(T* pWO)
{
    ...
    Cell cell(p);
    if(pWO->IsInWorld()) // need some clean up later
    {
        ASSERT(pWO->IsInGrid());
    ...
    sLog->outStaticDebug("Object %u enters grid[%u, %u]", GUID_LOPART(pWO->GetGUID()), cell.GridX(), cell.GridY());
...
void Map::Remove(Player* pPlayer, bool REMOVE)
{
    ...
    else
    {
        ...
        {
            sLog->outStaticDebug("Remove player %s from grid[%u, %u]", pPlayer->GetName(), cell.GridX(), cell.GridY());
            NGridType* grid = getNGrid(cell.GridX(), cell.GridY());
            ASSERT(grid != NULL);
