        ...
        void HandleScriptEffect(SpellEffIndex effIndex)
        {
            ...
            switch(effIndex)
            {
                ...
                case EFFECT_2:
                {
                    ...
                    for(Unit::AuraApplicationMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                    {
                        Aura* pAura = itr->second->GetBase();
                        SpellEntry const* pSpell = pAura->GetSpellProto();
                        if(pAura && pSpell->SpellIconID == 2007 && pAura->HasEffectType(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN))
                        {
                            pAura->ModStackAmount(-1, AURA_REMOVE_BY_ENEMY_SPELL);
                            // Remove dummys from rider (Necessary for updating visual shields)
                            if(Unit* pRider = target->GetCharmer())
                                if(Aura* pAura2 = pRider->GetAura(pAura->GetId()))
