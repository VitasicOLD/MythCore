    ...
    struct boss_sindragosaAI : public BossAI
    {
        ...
        void SpellHitTarget(Unit* target, SpellEntry const* spell)
        {
            ...
            if(Player* player = target->ToPlayer())
            {
                ...
                {
                    if(player->GetQuestStatus(QUEST_FROST_INFUSION) != QUEST_STATUS_REWARDED && spellId == spell->Id && (player->GetDifficulty(true) == RAID_DIFFICULTY_25MAN_NORMAL || player->GetDifficulty(true) == RAID_DIFFICULTY_25MAN_HEROIC))
                    {
...
public:
    ...
    {
        npc_spinestalkerAI(Creature* pCreature): ScriptedAI(pCreature), _instance(pCreature->GetInstanceScript())
        {
            me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);
            me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_GRIP, true);
        }
